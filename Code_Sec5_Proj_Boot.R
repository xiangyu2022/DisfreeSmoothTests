...
### Specify isometry and apply it to basis h and orthonormalzied score functions b
M1=6
isometry1=function(x) sqrt(densf(x,MLEf)/densg1(x,MLEg1))
l1h=list()
for (i in 1:(M1)){
  for (j in 1){
    call = paste("l",j,"h[[",i,"]]<-function(x) isometry",j,"(x) * hj[[",i+1,"]](G_f(x,MLEf))",sep="")
...
innerl1 = integrate(function(x) {sqrt(densf(x,MLEf)*densg1(x,MLEg1))},lower=L1,upper=U1,rel.tol=1e-8)$value
inner1lh = inner1lb = c()
for (i in 1:(M1)){
  inner1lh[i]=integrate(function(x) {l1h[[i]](x)*densg1(x,MLEg1)},lower=L1,upper=U1,rel.tol=1e-8)$value
}
for (i in 1:3){
  inner1lb[i]=integrate(function(x) {l1b[[i]](x)*densg1(x,MLEg1)},lower=L1,upper=U1,rel.tol=1e-8)$value
}
...
inner_step3_1_1 = integrate(function(x) {(norm_score_g11(x)-Kl1b[[1]](x))*Kl1b[[2]](x)*densg1(x,MLEg1)},lower=L1, upper=U1,rel.tol=1e-8)$value
inner_step3_2_1 = integrate(function(x) {norm_score_g11(x)*Kl1b[[1]](x)*densg1(x,MLEg1)},lower=L1 ,upper=U1,rel.tol=1e-8)$value
tildec2_g1 = function(x){Kl1b[[2]](x)-inner_step3_1_1/(1-inner_step3_2_1)*(norm_score_g11(x)-Kl1b[[1]](x))} # Same with below

inner_step3_c3 = integrate(function(x) {(norm_score_g11(x)-Kl1b[[1]](x))*Kl1b[[3]](x)*densg1(x,MLEg1)},lower=L1, upper=U1,rel.tol=1e-8)$value
Ua1c1c3 = function(x) {Kl1b[[3]](x) - inner_step3_c3/(1-inner_step3_2_1)*(norm_score_g11(x)-Kl1b[[1]](x))}
...
for (i in 1:(M1)){
  inner1_h[i]=integrate(function(x) {(norm_score_g11(x)-Kl1b[[1]](x))*Kl1h[[i]](x)*densg1(x,MLEg1)}, lower=L1, upper=U1, rel.tol=1e-6)$value
  call1 = paste("U1_h[[",i,"]]<-function(x) {Kl1h[[",i,"]](x)-inner1_h[",i,"]/(1-inner_step3_2_1)*(norm_score_g11(x)-Kl1b[[1]](x))}",sep="")
  ...
  call3 = paste("UKl1h[[",i,"]]<-function(x) UU1_h[[",i,"]](x)- inner_UU1_h[",i,"]/(1-inner_Ua3c3)*(norm_score_g13(x) - tildec3_g1(x))",sep="")
...
for (i in 1:M1){
  ...
  call003 = paste("inner",i,"_13=integrate(function(x) {hj[[",i+1
                  ,"]](G_f(x,MLEf))*norm_score_f3(x)*densf(x,MLEf) }, L1, U1, rel.tol = 1e-8)$value",sep="")
  call004 = paste("Kvj1[[",i+1,"]] = function(x) {UKl1h[[",i,"]](x) - norm_score_g11(x) * inner",i,
...
for (k in 1:B){
  ...
  for (j in 1:M1){
    vj_boot[k,j] <- sqrt(n)*mean(hj[[j+1]](G_g1(xx_boot,MLEg1)))
  }
...
res_tn_order_boot_K2_g1 <- res_tn_boot_K2_g1 <-res_gst_boot_K2_g1<-res_ks_boot_K2_g1 <- numeric(B)
vn_boot_K2_g1 <- matrix(NA,nrow=B,ncol=M1)
for (k in 1:B){
  xx_k2 = sampler_x1(n)
  for (j in 1:M1){
    vn_boot_K2_g1[k,j] <- sqrt(n)*mean(Kvj1[[j+1]](xx_k2))
  }
  res_ks_boot_K2_g1[k] <- max(abs(vn_boot_K2_g1[k,]))
...
par(mar = c(4, 5.5, 2, 2))
qqplot(res_tn_boot_g1,res_tn_boot_q, ylab=expression("Quantiles of "*hat(T)[n]*" from "*G[beta]),
...
par(mar = c(4, 5.5, 2, 2))
qqplot(res_tn_boot_K2_g1, res_tn_boot_q, ylab=expression("Quantiles of "*hat(T)[n]*" from "*G[beta]),
       xlab=expression("Quantiles of "*hat(T)[n]*" from "*F[gamma]*""),
       lwd = 2, cex.axis = 1.6, cex.lab = 1.8)
abline(0,1)
dev.off()
...
par(mar = c(4, 5.5, 2, 2))
qqplot(res_tn_boot_g1, res_tn_order_boot_q, ylab=expression("Quantiles of "*tilde(T)[n]*" from "*G[beta]),
...
par(mar = c(4, 5.5, 2, 2))
qqplot(res_tn_order_boot_K2_g1, res_tn_order_boot_q, ylab=expression("Quantiles of "*tilde(T)[n]*" from "*G[beta]),
...
print((sum(res_tn_boot_K2_g1>=res_tn_bs11)+1)/(length(res_tn_boot_K2_g1)+1))
print((sum(res_tn_order_boot_K2_g1>=res_tn_order_bs11)+1)/(length(res_tn_order_boot_K2_g1)+1))
